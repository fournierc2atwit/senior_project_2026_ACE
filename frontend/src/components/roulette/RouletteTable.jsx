...
// Physical wheel order for an American double-zero wheel (clockwise from top)
const WHEEL_ORDER = [
...
export default function RouletteTable({ onNavigate, playerName, initialChips, onSetChips }) {
  ...
  const handleSpin = async () => {
    ...
    try {
      ...
      // Always rotate forward — never backward — for a natural spin feel
      setRotation((prev) => {
      ...
      // Reveal the result after the spin animation finishes
      setTimeout(() => {
        setResult(res.data);
    ...
    <div className="rt-root">
          ...
          {WHEEL_ORDER.map((num, i) => {
            ...
            return (
              <span
                ...
                style={{
                  transform: `rotate(${angle}deg) translate(0, -${RADIUS_PX}px) rotate(${-angle}deg)`,
                }}
              >
                {num}
              </span>
      ...
      <div className="rt-controls">
        ...
        <div className="rt-value-picker">
          ...
          {betType === "color" && (
            <div className="rt-swatch-row">
              <button
                className={`rt-swatch rt-swatch-red ${betValue === "red" ? "rt-value-selected" : ""}`}
              ...
              <button
                className={`rt-swatch rt-swatch-black ${betValue === "black" ? "rt-value-selected" : ""}`}
          ...
          {betType === "parity" && (
            <div className="rt-swatch-row">
              <button
                className={`rt-simple-btn ${betValue === "odd" ? "rt-value-selected" : ""}`}
              ...
              <button
                className={`rt-simple-btn ${betValue === "even" ? "rt-value-selected" : ""}`}
          ...
          {betType === "dozen" && (
            <div className="rt-swatch-row">
              <button
                className={`rt-simple-btn ${betValue === 1 ? "rt-value-selected" : ""}`}
              ...
              <button
                className={`rt-simple-btn ${betValue === 2 ? "rt-value-selected" : ""}`}
                onClick={() => setBetValue(2)}
                disabled={spinning}
              >
                2nd 12
              </button>
              <button
                className={`rt-simple-btn ${betValue === 3 ? "rt-value-selected" : ""}`}
                onClick={() => setBetValue(3)}
                disabled={spinning}
              >
                3rd 12
              </button>
        ...
        <div className="rt-action-row">
          <button className="btn-clear" onClick={clearBet} disabled={amount === 0 || spinning}>
            Clear
          </button>
